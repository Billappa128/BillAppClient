...
import styles from "./Vcbchecking.module.css"
...
export default function Vcbchecking() {
    ...
    const imageOptions = [
        { value: 'image1', label: 'Hình Nền 1', path: images.vcbchecking },
    ...
    function insertDots(str) {
        const parts = str.match(/.{1,6}/g); // Chia chuỗi thành các phần có 6 kí tự
        if (!parts) return str;
        return parts.join('.'); // Nối các phần lại với dấu chấm
      }
    ...
    const [forms, setForms] = useState([
        {
            ...
            balance: 0,
            timeAgo: "",
            hash : insertDots(randomNumericString)
        },
    ]);
                    ...
                    <div ref={componentRef} className={`${styles.right} position-relative`}>
                        ...
                        <div className={`${styles.boxContent} position-absolute`}>
                            {forms.map((item, index) => (
                                <div className='my-2'>
                                    <div className={`${styles.topContent} d-flex align-items-center justify-content-between`}>
                                        <div className={`${styles.dateBox}`}>{getFormattedDate1(item.date)}</div>
                                    ...
                                    <div className={`${styles.tabContent}`} key={index}>
                                        <div className={styles.mainContent}>Số dư TK VCB: <span className={`${styles.receiverAccount}`}>{`${item.receiverAccount}`}</span>
                                            {item.colorAmount && <span className={`${styles.amountNumber}`}>{` +${item.formattedAmount} VND `}</span>}
                                            {!item.colorAmount && <span className={`${styles.amountNumber}`}>{` -${item.formattedAmount}VND `}</span>}
                                            <span className={`${styles.date}`}>{`lúc ${formatDate(item.date)} ${item.time}:10. `}</span>
                                            <span>{`Số dư ${item.balance} VND `}</span><span>{`Ref ${item.hash} ${diacritics.remove(item.description)}`}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
