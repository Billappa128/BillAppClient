...
import styles from "./Vpbanking.module.css"
import VP from "../../../../images/billbanking/ck-vpb.jpeg"
...
export default function Vpbanking() {
    ...
    const imageOptions = [
        { value: 'image1', label: 'Hình Nền 1', path: VP },
    ...
    const handleInputAmount = (e) => {
        ...
        const formattedAmount = isNaN(amount) ? "" : amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " "); // Định dạng giá trị có dấu phẩy
            ...
            <div className="content-body pb-5">
                ...
                <div className={styles.content}>
                    ...
                    <div ref={componentRef} className={`${styles.right} position-relative`}>
                        ...
                        <span className={`${styles.amountNumber} position-absolute`}><span className={styles.amountNumber1}>{`${formattedAmount}`}</span> <span className={`${styles.donvi} `}> {` ₫`}</span></span>
                        ...
                        {/*Thay icon  */}
                        {selectedBankData && <div className={`${styles.boxIcon} position-absolute `}><img src={selectedBankData.icon} className={`${styles.iconBank} `}  alt="iconBank" /></div>}
