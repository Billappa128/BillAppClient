...
import styles from "./Bidvbankingvip.module.css"
import BIDV1 from "../../../../images/billbanking/ck-bidv1.jpg"
...
export default function Bidvbankingvip() {
    ...
    const imageOptions = [
        { value: 'image1', label: 'Hình Nền 1', path: BIDV1 },
            ...
            <div className="content-body pb-5">
                ...
                <div className={styles.content}>
                    ...
                    <div ref={componentRef} className={`${styles.right} position-relative`}>
                        ...
                        <span className={`${styles.optionCus} position-absolute`}>{`Qúy khách đã chuyển thành công số tiền`}
                            <span> {`${formattedAmount} VND`} </span>đến số tài khoản
                            <span> {receiverAccount}</span>/<span> {diacritics.remove(receiverName)}</span>/ <span>{selectedBankData?.acbname}</span> vào lúc {formattedDate} {time}:17. Nội dung: {diacritics.remove(description)}
                        </span>
